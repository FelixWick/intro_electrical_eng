...
\fi
  ...
  \item Warum kann der Strom durch eine ideale Spule nicht sprunghaft geändert werden?
    \begin{itemize}[label=$\square$]
      \item[A)] Weil die magnetische Energie der Spule bei einem Stromsprung unendlich groß wäre.
      \item[B)] Weil eine sprunghafte Stromänderung eine unendliche induzierte Spannung erfordern würde.
      \item[C)] Weil Magnetfelder nicht instantan aufgebaut werden können.
      \item[D)] Weil die Induktivität vom Strom abhängt.
    \end{itemize}
...
\fi
  ...
  \item In welcher Situation ist die Zeigerdarstellung eines Stroms \emph{nicht} zulässig?
    \begin{itemize}[label=$\square$]
...
\fi
  ...
  \item Ein symmetrisches Drehstromsystem hat als praktischen Vorteil:
    \begin{itemize}[label=$\square$]
      ...
      \item[C)] die Erzeugung eines rotierenden Magnetfelds in elektrischen Maschinen
...
Berechnen Sie den Ersatzwiderstand des Netzwerks zwischen den Klemmen A und B.
